...
class _CourseDialogState extends State<CourseDialog> {
  ...
  void initState() {
    ...
    _codeController = TextEditingController(
      text: isEditing ? widget.course!.courseCode : 'CS102',
    );
    _titleController = TextEditingController(
      text: isEditing ? widget.course!.title : '',
    );
    _creditsController = TextEditingController(
      text: isEditing ? widget.course!.credits.toString() : '3',
    );
    _instructorController = TextEditingController(
      text: isEditing ? widget.course!.instructor : '',
    );
    _descriptionController = TextEditingController(
      text: isEditing ? widget.course!.description : '',
    );

    final validDepts = widget.departments.where((d) => d != 'All').toList();
    _selectedDepartment =
        isEditing && validDepts.contains(widget.course!.department)
  ...
  void _submit() {
    ...
    submitValidatedForm<Course>(
      ...
      validator: AppDependencyProvider.of(context).courseValidator,
  ...
  Widget build(BuildContext context) {
    ...
    return FormDialogShell(
      icon: isEditing ? Icons.edit_calendar : Icons.add_to_photos,
      title: isEditing ? 'Edit Course Record' : 'Add New Course',
      formKey: _formKey,
      onSubmit: _submit,
      submitLabel: isEditing ? 'Save Changes' : 'Add Course',
      fields: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            validator: (v) =>
                v == null || v.trim().isEmpty ? 'Instructor is required' : null,
          ),
          const SizedBox(height: 12),
          TextFormField(
